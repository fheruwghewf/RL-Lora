...
void ClassAEndDeviceLorawanMac::SetEnableCRDSA(bool enable) {
    m_enableCRDSA = enable;
    NS_LOG_FUNCTION(this << enable);
}

bool ClassAEndDeviceLorawanMac::GetEnableCRDSA() const {
...
ClassAEndDeviceLorawanMac::SendToPhy(Ptr<Packet> packetToSend)
{
    ...
    NS_ASSERT_MSG(m_phy, "LoRa PHY layer is not initialized!");
    //std::cerr<<" --- --- sendtophy --- ---"<<std::endl;
    

    // --- 1. 检查是否为 CRDSA 副本（通过 LoraPacketTag）---
    LoraTag crdsaTag;
    bool isCRDSAPacket = packetToSend->PeekPacketTag(crdsaTag);
    //std::cerr<<"SendToPhy: isCRDSAPacket= "<<isCRDSAPacket<<std::endl;
    // --- 2. 数据速率和功率调整（仅对非CRDSA副本或首次发送生效）---
    if (!isCRDSAPacket || m_retxParams.retxLeft == m_maxNumbTx - 1) 
    {
        // 原始 LoRaWAN 的 DR 适应逻辑（Confirmed Message 重传时降速）
        if (m_enableDRAdapt && (m_dataRate > 0) && 
            (m_retxParams.retxLeft < m_maxNumbTx) && 
            (m_retxParams.retxLeft % 2 == 0)) 
        {
            m_txPower = 14; // 重置功率
            m_dataRate = m_dataRate - 1; // 降速
        }
    }
    ...
    Ptr<LogicalLoraChannel> txChannel = GetChannelForTx();
    if (!txChannel) {
      Time backoff = Seconds(1); // 1秒后重试
      Simulator::Schedule(backoff, &ClassAEndDeviceLorawanMac::SendToPhy, this, packetToSend);
      return; // 退出当前发送
  }
    NS_ASSERT_MSG(txChannel, "No valid channel for transmission!");

    // std::cerr<<"Sending packet with SF" << unsigned(params.sf) << 
    //             ", TP=" << m_txPower << " dBm" <<
    //             (isCRDSAPacket ? " (CRDSA replica)" : "")<<std::endl;
    ...
    Time duration = m_phy->GetOnAirTime(packetToSend, params);
    m_channelHelper->AddEvent(duration, txChannel);
    NS_LOG_DEBUG("Channel " << txChannel->GetFrequency() << " Hz blocked for " << duration);

    // --- 6. 准备下行接收窗口（仅当非CRDSA副本或最后一个副本时）---
    if (!isCRDSAPacket || crdsaTag.GetReplicaId() == 0) 
    {
        m_phy->GetObject<EndDeviceLoraPhy>()->SetFrequency(txChannel->GetFrequency());
        uint8_t replyDataRate = GetFirstReceiveWindowDataRate();
        m_phy->GetObject<EndDeviceLoraPhy>()->SetSpreadingFactor(
    ...
    // std::cerr<<" --- --- finish sendtophy --- ---"<<std::endl;
}

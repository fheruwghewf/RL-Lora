...
EndDeviceLorawanMac::EndDeviceLorawanMac()
    : m_enableDRAdapt(false),
      ...
      m_enableCRDSA (false), // Enable CRDSA by default
      m_currentFCnt(0)
{
...
//     NS_LOG_FUNCTION(this << packet);
    ...
    // {
    //     postponeTransmission(netxTxDelay, packet);//过段时间再传
    //     return;
    // }
...
void
lorawan::EndDeviceLorawanMac::SetEnableCRDSA(bool enableCRDSA)
{
    m_enableCRDSA = enableCRDSA;
    NS_LOG_FUNCTION(this << enableCRDSA);
}

bool EndDeviceLorawanMac::GetEnableCRDSA() const {
...
{
    NS_LOG_FUNCTION(this << packet);
    // --- CRDSA 逻辑：跳过原始发送流程，直接进入时隙化副本发送 ---
    
    // std::cerr<<" --- --- send() --- --- GetEnableCRDSA(): "<< GetEnableCRDSA() <<std::endl;
    if (GetEnableCRDSA()) {
        //std::cerr<<"--- --- CRDSA send --- ---" <<std::endl;
        SendWithCRDSA(packet);
    } 
    else {
        // 原始ALOHA逻辑
    Time netxTxDelay = GetNextTransmissionDelay();
...
void
EndDeviceLorawanMac::SendWithCRDSA(Ptr<Packet> packet)
{
    // 1. 定义 CRDSA 参数
    uint32_t slotDurationMs = 5000;      // 时隙长度（5秒）
    uint8_t numReplicas = 3;             // 每个包的副本数
    Time now = Simulator::Now();           // 当前时间
    uint32_t currentSlot = now.GetMilliSeconds() / slotDurationMs;  // 当前时隙索引
    uint64_t packetId = GenerateUniqueId();
     // 2. 首先发送原始数据包（不标记为副本）
     
        // 选择下一个时隙发送原始包
        uint32_t targetSlot = currentSlot + 1;
        Time sendTime = MilliSeconds(targetSlot * slotDurationMs);

        // 检查 duty cycle 限制
        Time nextTxDelay = GetNextTransmissionDelay();
        if (nextTxDelay > Seconds(0))
        {
            NS_LOG_WARN("Duty cycle violation, postponing original packet");
            Simulator::Schedule(nextTxDelay, &EndDeviceLorawanMac::SendWithCRDSA, this, packet);
            return; // 直接返回，等延迟过后重新发送整个包和副本
        }
        ...
        LoraTag tag;
        originalPacket->RemovePacketTag(tag);   // 移除原有标记（如有）
        tag.SetReplicaId(0); // 原始包可以使用0作为特殊ID
        tag.SetSlotIndex(targetSlot);
        tag.SetIsReplica(false); // 明确标记为非副本
        tag.SetCurrentSlot(currentSlot); // 设置当前时隙
        tag.SetPacketId(packetId);
        originalPacket->AddPacketTag(tag);

        // 调度发送原始包
        // std::cerr << "CRDSA: scheduling original packet in slot " << targetSlot <<" ,packetId= "<<packetId<<" ,currentSlot= "<<currentSlot<< std::endl;
        Simulator::Schedule(sendTime - now, &EndDeviceLorawanMac::DoSend, this, originalPacket);
    


    // 2. 随机选择多个时隙发送副本
    for (uint8_t replicaId = 1; replicaId <= numReplicas; replicaId++)
    {
        // 随机选择时隙（避免所有副本集中在相邻时隙）
        uint32_t targetSlot = currentSlot + 1 + replicaId * 2 + rand() % 2;
        Time sendTime = MilliSeconds(targetSlot * slotDurationMs);

        // 3. 检查 duty cycle 限制（需继承原有逻辑）
        Time nextTxDelay = GetNextTransmissionDelay();
        if (nextTxDelay > Seconds(0))
        {
            NS_LOG_WARN("Duty cycle violation, postponing replica " << replicaId);
            Simulator::Schedule(nextTxDelay, &EndDeviceLorawanMac::SendWithCRDSA, this, packet);
            continue;
        }

        // 4. 为副本添加标记（存储副本ID和目标时隙）
        Ptr<Packet> replica = packet->Copy();
        LoraTag tag;
        replica->RemovePacketTag(tag);   // 移除原有标记（如有）
        tag.SetReplicaId(replicaId);
        tag.SetSlotIndex(targetSlot);
        tag.SetIsReplica(true);  // 明确标记为副本
        tag.SetCurrentSlot(currentSlot); // 设置当前原始时隙
        tag.SetPacketId(packetId);
        replica->AddPacketTag(tag);

        // 5. 调度发送
        // std::cerr << "CRDSA: scheduling replica packet in slot " << targetSlot <<" ,packetId= "<<packetId<<" ,currentSlot= "<<currentSlot<< std::endl;
        Simulator::Schedule(sendTime - now, &EndDeviceLorawanMac::DoSend, this, replica);
    }
}


uint64_t EndDeviceLorawanMac::GenerateUniqueId() {
    // 使用NS-3仿真时间（纳秒级）
    ...
    uv->SetAttribute("Max", DoubleValue(65535));
    uint32_t rand_num = uv->GetInteger();
    
    return (time << 16) | rand_num;
}
...
//     if (packet != m_retxParams.packet)  // this is a new packet
//     {
//         NS_LOG_DEBUG(
...
{
    NS_LOG_FUNCTION(this << packet);
    // --- 1. 检查是否已包含协议头（通过检查是否存在 LoraTag）---
    LoraTag crdsaTag;
    bool isCRDSAPacket = packet->PeekPacketTag(crdsaTag);
    //std::cerr<<"Dosend: isCRDSAPacket= "<<isCRDSAPacket<<std::endl;
    // --- 2. 新包处理（非重传且非CRDSA副本）---
    

    if (packet != m_retxParams.packet) 
    {
        //NS_LOG_DEBUG("Processing new application packet");
         m_currentFCnt++;
        // NS_LOG_DEBUG("APP packet: " << packet << ".");

        // 添加帧头和MAC头 :避免重复添加协议头（CRDSA 副本已包含头信息）
        LoraFrameHeader frameHdr;
        ApplyNecessaryOptions(frameHdr);
        packet->AddHeader(frameHdr);
                ...
                // 检查负载大小
        if (packet->GetSize() > m_maxAppPayloadForDataRate.at(m_dataRate)) {
            NS_LOG_WARN("Packet too large for current DR, canceling transmission");
    ...
    // --- 3. 重传包处理 ---
    else if (packet == m_retxParams.packet) 
    {
        NS_LOG_DEBUG("Processing retransmission packet");
        std::cerr<<" --- --- retransmission packet --- ---"<<std::endl;
        ...
        if (!isCRDSAPacket) {
            ...
            m_retxParams.retxLeft = m_retxParams.retxLeft - 1;
                // --- 4. 最终发送（所有路径最终汇聚到这里）---
    NS_LOG_DEBUG("Sending packet to PHY layer" << 
        (isCRDSAPacket ? " (CRDSA replica)" : "") <<
        (packet == m_retxParams.packet ? " (retransmission)" : ""));
    //std::cerr<<"Sending packet to PHY layer" << 
        // (isCRDSAPacket ? " (CRDSA replica)" : "") <<
        // (packet == m_retxParams.packet ? " (retransmission)" : "")<<std::endl;
    SendToPhy(packet);
    }
                                  ...
                                  int repetitions)
{
    ...
    bool channelMaskOk = true;  //信道掩码
    bool dataRateOk = true;  //数据速率
    bool txPowerOk = true;  //发射功率

    // Check the channel mask
    /////////////////////////
    // Check whether all specified channels exist on this device
    auto channelList = m_channelHelper->GetChannelList();  //获取设备的可用信道列表
    int channelListSize = channelList.size();  //获取设备的信道数量

    for (auto it = enabledChannels.begin(); it != enabledChannels.end(); it++)  //检查请求的信道掩码是否在设备的可用信道范围内。
    {
    ...
    // Check the dataRate  检查请求的数据速率是否有效。
    /////////////////////
    ...
    uint8_t sf = GetSfFromDataRate(dataRate);  //将数据速率转换为扩频因子（SF）和带宽（BW）
    double bw = GetBandwidthFromDataRate(dataRate);
    NS_LOG_DEBUG("SF: " << unsigned(sf) << ", BW: " << bw);
    if (sf == 0 || bw == 0)//如果转换结果为 0，表示数据速率无效，标记 dataRateOk 为 false。
    {
        dataRateOk = false;
    ...
    // specified dataRate.检查请求的数据速率是否在至少一个启用信道的允许范围内。
    //
    if (dataRateOk && channelMaskOk) // If false, skip the check
    {
        bool foundAvailableChannel = false;
        for (auto it = enabledChannels.begin(); it != enabledChannels.end(); it++)//遍历启用的信道列表，检查数据速率是否在当前信道的最小和最大数据速率范围内
        {
            ...
            {
                foundAvailableChannel = true;//若在，则找到了支持该速率的信道
                break;
            }
        }

        if (!foundAvailableChannel)//如果找不到支持该数据速率的信道，标记 dataRateOk 为 false。
        {
            dataRateOk = false;
    ...
    if (GetDbmForTxPower(txPower) == 0)//检查请求的发射功率是否有效.将发射功率转换为 dBm 值。如果转换结果为 0，表示发射功率无效，标记 txPowerOk 为 false。
    {
        txPowerOk = false;
    }
    ...
    // If all checks are successful, set parameters up如果所有检查通过，更新设备的信道掩码、数据速率和发射功率。
    //////////////////////////////////////////////////
    if (channelMaskOk && dataRateOk && txPowerOk)
    {
        // Cycle over all channels in the list
        for (uint32_t i = 0; i < m_channelHelper->GetChannelList().size(); i++)//遍历设备的可用信道列表，(以启用或禁用信道）
        {
            if (std::find(enabledChannels.begin(), enabledChannels.end(), i) !=
                enabledChannels.end())//当前信道的索引 i是否在启用列表（enabledChannels）之中
            {
                m_channelHelper->GetChannelList().at(i)->SetEnabledForUplink();//若在，表示该信道需要启用，调用 SetEnabledForUplink() 启用该信道。
                NS_LOG_DEBUG("Channel " << i << " enabled");
            }
            else
            {
                m_channelHelper->GetChannelList().at(i)->DisableForUplink();//若不在，调用 DisableForUplink() 禁用该信道
                NS_LOG_DEBUG("Channel " << i << " disabled");
            }
        }

        // Set the data rate更新设备的数据速率和发射功率。
        m_dataRate = dataRate;
    ...
    // Craft a LinkAdrAns MAC command as a response生成 LinkAdrAns MAC 命令，通知网络服务器请求的执行结果。
    ///////////////////////////////////////////////
    //终端使用 LinkADRAns 命令来应答 LinkADRReq 命令

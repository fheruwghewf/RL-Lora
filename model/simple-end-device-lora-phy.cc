 ...
 */
                               ...
                               double frequencyMHz, double txPowerDbm)
 {
   ...
   NS_LOG_INFO ("Current state: " << m_state);
    //std::cerr<<"channel->send()"<<std::endl;
   //**** tests
  ...
  if (packet->PeekPacketTag(tag)) {
      isReplica = tag.GetIsReplica();
  }
  // 仅统计原始包（非副本）
  if (!isReplica) {
      global_sent++;
      std::cerr << " global_sent = " << global_sent << std::endl;
  }
  ...
  //std::cerr<<"finish channel->send(),global_sent= "<<global_sent<<std::endl;

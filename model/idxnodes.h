...
struct LabDevices{
	...
	// int state_i[3] = {7,0,0}; // 'SF' , 'TXPwr', 'Freq'
	// int state_ii[3] = {7,0,0};
	
	int state_i[4] = {7,0,0,0}; // 'SF' , 'TXPwr', 'Freq','crdsa'
	int state_ii[4] = {7,0,0,0};

	double pwr_i = 0.01;
	double pwr_ii = 0.01;
	double pwr_total = 0.0;  // 总功耗
	...
	int fail = 0;   // 失败次数
	int sent_packets = 0; // 发送的包数

	float distance = 0.0;   //// 设备到网关的距离
	...
	int pkg_on_air = 2; //nodeid 当前 正在空中传输的LoRa数据包数量（即已发送但尚未被网关完全接收的数据包）。
...
extern struct LabDevices trialdevices[5000];
